...
```{r}
dt <- rbindlist(list(fread("unzip -cq ./data/raw_data/raw_dt_replicate1.csv", nThread = 4),
                     fread("unzip -cq ./data/raw_data/raw_dt_replicate2.csv", nThread = 4)))
dt_desc <- fread("data/experiment_description.csv")

dt[, Stimulation := str_extract(track_id_uni, "[A-Z]+")]
...
dt_len[, Stimulation := str_extract(Uniq, "[A-Z]+")]
```
...
ids_to_keep <- dt[Uniq %in% dt_len[len >= len_threshold, Uniq], sample(unique(Uniq), size = 7), by = Stimulation]$V1
...
temp <- dt_plot[, .(Uniq=unique(Uniq)), by = Stimulation][, .(Uniq, dummy_traj_number=seq(1:nrow(.SD))), by = Stimulation]
dt_plot <- merge(dt_plot, temp, by=c("Stimulation", "Uniq"))

dt_plot[, Stimulation := factor(Stimulation, levels = c("CTR", "IGF", "HRG", "HGF", "BTC", "EPR", "EGF", "ARG", "TGF"))]
...
## Trim out early response

Get rid of the first 100 frames to ignore early acute response to GF. This might bias the network to only look for it. Don't do it here, but while loading data for CNN training. Export only cells with IDS which would still be longer than thresholf after trimming 100 frames.

```{r}
dt_clipped <- dt[Time >= 100]
dt_len_clipped <- dt_clipped[, .(len = nrow(.SD)), by=Uniq]
dt_len_clipped <- merge(dt_len_clipped, dt_clipped[, .(Uniq, Stimulation)], by="Uniq")
dt_len_clipped <- dt_len_clipped[, .SD[1], by = "Uniq"]
clip_length(dt_len_clipped)
```


## Export

Do not export results for amphiregulin and TGFalpha, this was a test for further experiment.
...
dt_export <- dt[Uniq %in% dt_len[len >= len_threshold, Uniq], .(Uniq, Stimulation, Time, Ratio_ERK, Ratio_AKT)]
dt_export <- dt_export[!Stimulation %in% c("ARG", "TGF")]
dt_export <- dcast(dt_export, Uniq + Stimulation ~ Time, value.var = c("Ratio_ERK", "Ratio_AKT"))
colnames(dt_export) <- str_replace(colnames(dt_export), "Ratio_", "")
setnames(dt_export, c("Uniq", "Stimulation"), c("ID", "class"))
...
## Export cells with trimmed IDs

Make a second dataset with only cell IDs from which could trim 100 frames and still have length above threhsold.
```{r eval=FALSE}
# Cast to wide with proper column names
dt_export <- dt[Uniq %in% dt_len_clipped[len >= len_threshold, Uniq], .(Uniq, Stimulation, Time, Ratio_ERK, Ratio_AKT)]
dt_export <- dt_export[!Stimulation %in% c("ARG", "TGF")]
dt_export <- dcast(dt_export, Uniq + Stimulation ~ Time, value.var = c("Ratio_ERK", "Ratio_AKT"))
colnames(dt_export) <- str_replace(colnames(dt_export), "Ratio_", "")
setnames(dt_export, c("Uniq", "Stimulation"), c("ID", "class"))
